...
const std::string APP_NAME = "FoxgloveRawimage";
...
void process_image_data(dds::sub::DataReader<::foxglove::RawImage> reader)
{
...
void run(std::shared_ptr<DDSParticipantSetup> participant_setup)
{
    ...
    auto image_reader = std::make_shared<DDSReaderSetup<::foxglove::RawImage>>(
            participant_setup,
            topics::IMAGE_TOPIC,
            qos_profiles::LARGE_DATA_SHMEM);

    // Setup Writer Interface with LARGE_DATA_SHMEM QoS
    auto image_writer = std::make_shared<DDSWriterSetup<::foxglove::RawImage>>(
            participant_setup,
            topics::IMAGE_TOPIC,
            qos_profiles::LARGE_DATA_SHMEM);
    ...
    while (!application::shutdown_requested) {
        try {
            ...
            snprintf(
                    image_id_buffer,
                    sizeof(image_id_buffer),
                    "img_%06u",
                    image_count);

            // Populate image metadata
            image_msg.frame_id(image_id_buffer);
            image_msg.width(IMAGE_WIDTH);
            image_msg.height(IMAGE_HEIGHT);
            image_msg.encoding("rgb8");
            image_msg.step(IMAGE_WIDTH*3);
            ...
            std::cout << "[IMAGE_PUBLISHER] Published Image - ID: "
                      << image_msg.frame_id()
            ...
            rti_logger.notice(
                    ("Published Image - id:" + std::string(image_msg.frame_id())
                     + ", size:" + std::to_string(image_msg.data().size())
                     + " bytes" + ", " + std::to_string(IMAGE_WIDTH) + "x"
