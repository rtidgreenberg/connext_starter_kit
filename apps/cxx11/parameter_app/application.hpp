...
namespace application {
    ...
    struct ApplicationArguments {
        ...
        ApplicationArguments(
            ...
            const std::string& params_file_path_param,
            const std::string& node_name_param,
            const std::string& target_name_param,
            bool server_mode_param)
            : parse_result(parse_result_param),
            domain_id(domain_id_param),
            verbosity(verbosity_param),
            qos_file_path(qos_file_path_param),
            params_file_path(params_file_path_param),
            node_name(node_name_param),
            target_name(target_name_param),
            server_mode(server_mode_param) {}
    };
    ...
    inline ApplicationArguments parse_arguments(int argc, char *argv[])
    {
        ...
        std::string params_file_path = "apps/cxx11/parameter_app/params.yaml";
        std::string node_name = "parameter_server";
        std::string target_name = "parameter_server";
        ...
        while (arg_processing < argc) {
            ...
            || strcmp(argv[arg_processing], "--params-file") == 0)) {
                params_file_path = argv[arg_processing + 1];
                arg_processing += 2;
            } else if ((argc > arg_processing + 1)
            && (strcmp(argv[arg_processing], "-n") == 0
            || strcmp(argv[arg_processing], "--node-name") == 0)) {
                node_name = argv[arg_processing + 1];
                arg_processing += 2;
            } else if ((argc > arg_processing + 1)
            && (strcmp(argv[arg_processing], "-t") == 0
            || strcmp(argv[arg_processing], "--target") == 0)) {
                target_name = argv[arg_processing + 1];
                arg_processing += 2;
            } else if (strcmp(argv[arg_processing], "-s") == 0
            || strcmp(argv[arg_processing], "--server") == 0) {
                server_mode = true;
                arg_processing += 1;
            } else if (strcmp(argv[arg_processing], "-h") == 0
            || strcmp(argv[arg_processing], "--help") == 0) {
                std::cout << "Parameter application - demonstrates ROS2-style parameters over DDS." << std::endl;
                show_usage = true;
        ...
        if (show_usage) {
            ...
            "    -p, --params-file  <str>   Path to YAML parameters file.\n"\
            "                               Default: apps/cxx11/parameter_app/params.yaml\n"            "    -n, --node-name    <str>   Unique name for this server instance.\n"\
            "                               Default: parameter_server\n"
            "    -t, --target       <str>   Name of the server to send requests to (client mode).\n"\
            "                               Default: parameter_server\n"            "    -s, --server               Run as parameter server (responds to requests).\n"
            "                               Default: client mode (sends requests)\n"
            << std::endl;
        }

        return ApplicationArguments(parse_result, domain_id, verbosity, qos_file_path, params_file_path, node_name, target_name, server_mode);
    }
