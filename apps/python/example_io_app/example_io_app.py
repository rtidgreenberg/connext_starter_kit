...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '..', 'dds', 'python'))
...
async def process_position_data(reader):
  ...
  async for data in reader.take_data_async():
    ...
    # Log position data
    distlog.Logger.info(f"Received Position data - source:{data.source_id}, lat:{data.latitude}, lon:{data.longitude}, alt:{data.altitude}")
...
class ExampleIOApp:
    ...
    @staticmethod
    async def run(domain_id: int):

        app_name = "Example Python IO App"

        # Load QoS profiles from the DDS_QOS_PROFILES.xml file
        qos_file_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'dds', 'qos', 'DDS_QOS_PROFILES.xml')
        ...
        participant_qos = qos_provider.participant_qos_from_profile(dds_config.DEFAULT_PARTICIPANT_QOS)

        participant_qos.participant_name.name = app_name
        
        participant = dds.DomainParticipant(domain_id, participant_qos)
        
        print(f"DomainParticipant created with QoS profile: {dds_config.DEFAULT_PARTICIPANT_QOS}")
        ...
        print(f"RTI Distributed Logger configured for domain {domain_id} with application kind: {app_name}")
        ...
        button_topic = dds.Topic(
            participant, topics.BUTTON_TOPIC, example_types.Button)
            
        config_topic = dds.Topic(
            participant, topics.CONFIG_TOPIC, example_types.Config)
        ...
        position_reader_qos = qos_provider.set_topic_datareader_qos(dds_config.ASSIGNER_QOS,
            topics.POSITION_TOPIC)
        position_reader = dds.DataReader(participant.implicit_subscriber, position_topic, position_reader_qos)
        
        # Create DataWriters with QoS configured from DDS_QOS_PROFILES.xml
        command_writer_qos = qos_provider.set_topic_datawriter_qos(dds_config.ASSIGNER_QOS,
            topics.COMMAND_TOPIC)
        command_writer = dds.DataWriter(participant.implicit_publisher, command_topic, command_writer_qos)
        
        button_writer_qos = qos_provider.set_topic_datawriter_qos(dds_config.ASSIGNER_QOS,
            topics.BUTTON_TOPIC)
        button_writer = dds.DataWriter(participant.implicit_publisher, button_topic, button_writer_qos)
        
        config_writer_qos = qos_provider.set_topic_datawriter_qos(dds_config.ASSIGNER_QOS,
            topics.CONFIG_TOPIC)
        config_writer = dds.DataWriter(participant.implicit_publisher, config_topic, config_writer_qos)
        
        print("[SUBSCRIBER] RTI Asyncio reader configured for Position data...")
        print("[PUBLISHER] RTI Asyncio writers configured for Command, Button, and Config data...")
        ...
        async def publisher_task():
            ...
            while True:
                try:
                    ...
                    command_sample.destination_id = "target_system"
                    command_sample.command_type = example_types.CommandType.COMMAND_START
                    ...
                    print(f"[COMMAND_PUBLISHER] Published Command - ID: {command_sample.command_id}")
                    distlog.Logger.info(f"Published Command - id:{command_sample.command_id}, type:{command_sample.command_type}")
                    
                    # Publish Button message
                    button_sample = example_types.Button()
                    button_sample.source_id = app_name
                    button_sample.button_id = "btn_1"
                    button_sample.button_state = example_types.ButtonState.BUTTON_PRESSED
                    ...
                    print(f"[BUTTON_PUBLISHER] Published Button - ID: {button_sample.button_id}, Count: {button_count}")
                    distlog.Logger.info(f"Published Button - id:{button_sample.button_id}, state:{button_sample.button_state}, count:{button_count}")
                    
                    # Publish Config message
                    config_sample = example_types.Config()
                    config_sample.destination_id = "config_target"
                    ...
                    print(f"[CONFIG_PUBLISHER] Published Config - Parameter: {config_sample.parameter_name}")
                    distlog.Logger.info(f"Published Config - parameter:{config_sample.parameter_name}, value:{config_sample.parameter_value}")
        ...
        try:
            await asyncio.gather(
                publisher_task(),
                process_position_data(position_reader),
                main_task()
            )
...
def main():
    ...
    try:
        # Run
        rti.asyncio.run(ExampleIOApp.run(domain_id=args.domain_id))
    except KeyboardInterrupt:
        pass
