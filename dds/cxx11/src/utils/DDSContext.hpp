...
public:
      ...
      const dds::core::status::SampleRejectedStatus &status)
  {
    std::cout << reader.topic_name() 
              << "ParticipantListener: on_sample_rejected()" 
              << std::endl;
  }
      ...
      const dds::core::status::LivelinessChangedStatus &status)
  {
    ...
    if (status.not_alive_count_change() > 0)
    {
      ...
      // You can use reader.matched_publication_data(handle) to get more info
    }
  }
  ...
  {
    std::cout << reader.topic_name() 
              << "ParticipantListener: on_sample_lost()" 
              << std::endl;
  }
...
public:
  DDSContext(const int domain_id, 
      ...
      : _domain_id(domain_id),
        _async_waitset(AsyncWaitSetProperty().thread_pool_size(thread_pool_size))
  {
    try
    {
      if (!participant_qos_file.empty() && !participant_qos_profile.empty())
      {
        ...
        auto participant_qos = qos_provider.participant_qos(participant_qos_profile);

        // Name the Participant so it's easier to track
        participant_qos << rti::core::policy::EntityName().name(app_name);

        // Create Participant
        _participant = dds::domain::DomainParticipant(
                                   _domain_id, participant_qos);

        std::cout << "DDSContext created with QoS profile: "
                  << participant_qos_profile
                  << " from file: " 
                  << participant_qos_file
                  << "and Domain ID: "
                  << domain_id
                  << std::endl;
      }
      else
      {
        ...
        std::cout << "DDSContext created with default QoS"
                  << std::endl;
      }
    }
    ...
    try
    {
      ...
      std::cout << "RTI Distributed Logger configured for domain " << _domain_id << std::endl;
                
      // Log application startup event
      _dist_logger.info("DDSContext initialized with distributed logging enabled");
    }
    catch (const std::exception& ex)
    {
      std::cerr << "Failed to setup distributed logger: " << ex.what() << std::endl;
  ...
  ~DDSContext()
  {
    ...
    {
      _async_waitset.stop();
      std::cout << "AsyncWaitSet stopped during DDSContext destruction" << std::endl;
    }
    catch (const std::exception &e)
    {
      std::cerr << "Error stopping AsyncWaitSet during destruction: " << e.what() << std::endl;
    }
    ...
    // The DomainParticipant will be destroyed automatically when _participant goes out of scope
    // Don't call finalize_participant_factory() here - it should be done at application level
    // after ALL DDSContext instances are destroyed
  ...
  void stop_async_waitset()
  {
    ...
    {
      std::cerr << "Error stopping AsyncWaitSet: " << e.what() << std::endl;
    }
  }
  ...
  void shutdown()
  {
    ...
    std::cout << "DDSContext shutdown initiated for domain " << _domain_id << std::endl;
  }
