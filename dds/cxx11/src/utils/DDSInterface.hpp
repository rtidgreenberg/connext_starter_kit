...
public:
  ...
  using DataProcessingFunction = std::function<void(dds::sub::DataReader<T>&)>;
        ...
        _qos_profile(qos_profile)
  {
    ...
    case KIND::WRITER:
      ...
      if (!_qos_file.empty() && !_qos_profile.empty())
      {
        ...
        std::cout << "DataWriter created on topic: "
                  << _topic_name << " with QoS profile: " << _qos_profile
                  << std::endl;
      }
      else
      {
    ...
    case KIND::READER:
      ...
      if (!_qos_file.empty() && !_qos_profile.empty())
      {
        ...
        std::cout << "DataReader created on topic: "
                  << _topic_name 
                  << " with QoS profile: " 
                  << _qos_profile
                  << std::endl;
  ...
  {
    if (!handler) {
      std::cerr << "Error: No data handler provided for AsyncWaitSet" << std::endl;
      return;
    }
    ...
    if (_reader != dds::core::null)
    {
      ...
      _condition =
          dds::core::cond::StatusCondition(_reader);
    }
    ...
    if (_kind == KIND::READER)
    {
      ...
      // Attach conditions. The Async Waitset will be triggered when the attached
    ...
    {
      std::cerr << "Error: Attempt to enable Async Waitset on Writer Interface" << std::endl;
      return;
    }
  }
  ...
  dds::pub::DataWriter<T> writer() const
  {
    if (_kind != KIND::WRITER) {
      std::cerr << "Warning: Attempting to get writer from READER interface" << std::endl;
    }
    return _writer;
  }
  ...
  {
    if (_kind != KIND::READER) {
      std::cerr << "Warning: Attempting to get reader from WRITER interface" << std::endl;
    }
    return _reader;
  }
