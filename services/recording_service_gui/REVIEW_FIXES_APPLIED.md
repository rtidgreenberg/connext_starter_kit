...
`python_types/` is now a valid generated directory — `setup.sh` generates
Python type modules from monitoring IDL using `rtiddsgen -language Python`
(requires `rti.connext >= 7.3.1` which includes `idl.xtypes_compliance`).
The directory is in `.gitignore` since it is regenerated by `setup.sh`.
...
- `xml_types/`, `python_types/` (generated files)
