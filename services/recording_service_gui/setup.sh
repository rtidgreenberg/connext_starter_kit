...
#   1. Convert admin IDL files (ServiceAdmin, ServiceCommon,
#      RecordingServiceTypes) to XML for use with DynamicData / QosProvider
#      (required for CDR serialization in the request/reply admin API).
#   2. Generate Python type modules for the monitoring IDL files
#      (ServiceMonitoring, RecordingServiceMonitoring, RoutingServiceMonitoring)
#      so they can be imported directly (requires rti.connext >= 7.3.1).
#
...
# Admin IDL files — converted to XML for DynamicData CDR serialization
ADMIN_IDL_FILES=("ServiceCommon.idl" "ServiceAdmin.idl" "RecordingServiceTypes.idl")

# Monitoring IDL files — generated as Python type modules (rti.connext >= 7.3.1)
MONITORING_IDL_FILES=("ServiceCommon.idl" "RecordingServiceMonitoring.idl"
                      "RoutingServiceMonitoring.idl" "ServiceMonitoring.idl")
...
# --- Convert admin IDL to XML (for DynamicData / CDR serialization) ---
mkdir -p "$XML_OUT_DIR"
echo
echo "Converting admin IDL files to XML..."

for idl in "${ADMIN_IDL_FILES[@]}"; do
...
# --- Generate Python type modules for monitoring IDL ---
PYTHON_TYPES_DIR="$SCRIPT_DIR/python_types"
mkdir -p "$PYTHON_TYPES_DIR"
echo
echo "Generating Python type modules for monitoring IDL..."

for idl in "${MONITORING_IDL_FILES[@]}"; do
    echo "  Generating $idl → Python..."
    "$RTIDDSGEN" -language Python -d "$PYTHON_TYPES_DIR" -I "$IDL_DIR" "$IDL_DIR/$idl" -replace 2>&1
done

echo
echo "✓ Python type modules generated in: $PYTHON_TYPES_DIR"
ls -la "$PYTHON_TYPES_DIR"/*.py
