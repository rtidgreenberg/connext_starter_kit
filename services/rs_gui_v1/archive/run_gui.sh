#!/bin/bash
# Run script for Recording Service GUI Control (Python)
# Handles environment setup and launches the tkinter GUI
...
echo "=== Recording Service GUI Control ==="
echo
...
if [ ! -f "$XML_DIR/ServiceAdmin.xml" ] || \
   [ ! -f "$XML_DIR/RecordingServiceTypes.xml" ] || \
   [ ! -f "$XML_DIR/ServiceMonitoring.xml" ]; then
...
# --- Check QoS files ---
if [ ! -f "$SCRIPT_DIR/MonitoringSubscriber_QOS_PROFILES.xml" ]; then
    echo "ERROR: MonitoringSubscriber_QOS_PROFILES.xml not found in $SCRIPT_DIR"
    exit 1
fi
echo "✓ QoS profile files found"
...
echo "----------------------------------------------"
cd "$SCRIPT_DIR"
python3 recording_service_gui.py --nddshome "$NDDSHOME" "$@"
