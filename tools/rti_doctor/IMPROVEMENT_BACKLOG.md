...
| HAR-6 | Re-verify the vendor e2e tier after the HAR-2 domain migration. | The 2026-08-06 review follow-up edited all six vendor suites to use `domains.for_suite`, and the unit and live tiers were re-run but the vendor tier was not. Docker and the Fast DDS images are present on the development host, so there is no blocker other than that it was not done. | Run `./run_tests.sh vendor`. Fix any suite whose fixture assumed the old private `DOMAIN_BASE` value or a per-run-varying domain. | The vendor tier passes, or each failure is triaged to a cause and recorded here. Until then, treat those six suites as unverified against current `main`. |
